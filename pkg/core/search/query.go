...
func resolveSearchQuery(driver string, q interface{}, condition Condition) {
	...
	if qType.Kind() == reflect.Ptr || qType.Kind() == reflect.Pointer {
		if qValue.IsNil() {
			return
		}
		qType = qType.Elem()
...
func pgSql(t *resolveSearchTag, condition Condition, qValue reflect.Value, i int) {
	switch t.Type {
	case "exact", "iexact":
